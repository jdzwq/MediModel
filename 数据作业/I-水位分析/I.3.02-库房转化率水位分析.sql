...
-----------------------------------------------------------------------------------------------
--脚本：季度物资转化率
--说明：转化率 = 销售金额 / （库存金额 - 购入金额 + 销售金额)
-----------------------------------------------------------------------------------------------
--汇总水位值
drop table TMP_水位 purge;
create table TMP_水位 nologging as
select 机构编码, 代码, 业务期间 季度, round(sum(销售金额) / (sum(库存金额) - sum(购入金额) + sum(销售金额)),2) 转化率
from(
select a.机构编码
    ,a.代码
    ,case
...
(
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 转化率) 
...
and exists (select 1 from TMP_分位 b where a.代码 = b.代码 and a.转化率 <= b.中分位)
)t where s.代码 = t.代码);
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('库房') and 特征类 = '库房转化率';
commit;
...
where a.代码 = b.代码 and a.转化率 < b.高分位 - b.分位矩 * 1.5
order by 特征名,特征位;
commit;
...
and exists(
    select 1 from 线索_问题特征 t where t.特征类 = '库房转化率' 
