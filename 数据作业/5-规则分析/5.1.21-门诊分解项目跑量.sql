...
from 临时_门诊诊疗 g inner join 规则_诊疗分解 t on (regexp_instr(upper(g.代码), t.分项编码) > 0 or regexp_instr(g.名称, t.分项名称) > 0)
...
select '门诊' 就医来源,
    ...
    机构编码,机构名称,门诊科室,门诊医生,身份证号,姓名,性别,年龄,门诊日期,门诊天数,疾病诊断,
    类别, 代码, 名称, 规格, 单位, 单价, 人次, 天数, 频次, 剂量, 数量, 金额, 列支, 日期
from 临时_门诊材料 g inner join 规则_材料分解 t on regexp_instr(g.名称, t.分项名称) > 0
where exists (select 1 from 临时_计费项目 s where g.机构编码 = s.机构编码 and g.身份证号 = s.身份证号 and g.日期 = s.日期 and g.名称 = s.名称)
order by 门诊科室,身份证号,门诊日期,名称;
commit;
--*******************************************************************************************--
