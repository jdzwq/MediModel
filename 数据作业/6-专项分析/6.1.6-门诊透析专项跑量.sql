...
from 临时_门诊诊疗 g
inner join 规则_诊疗限制 t on upper(g.代码) like t.诊疗编码 || '%' and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0 
...
where exists (select 1 from 规则_诊疗限制 t where upper(g.代码) like t.诊疗编码 || '%' and t.就医限制 in ('住院') 
    and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0 )
order by 门诊科室,身份证号,门诊日期,名称;
commit;
-----------------------------------------------------------------------------------------------
...
from 临时_门诊诊疗 g where g.性别 <> '女' 
    ...
    and t.性别限制 in ('女性') and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0 )
order by 门诊科室,身份证号,门诊日期,名称;
commit;
-----------------------------------------------------------------------------------------------
...
from 临时_门诊诊疗 g where g.年龄 > 1
    and exists (select 1 from 规则_诊疗限制 t where upper(g.代码) like t.诊疗编码 || '%' 
    and t.年龄限制 in ('新生儿') and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0 )
order by 门诊科室,身份证号,门诊日期,名称;
commit;
...
) g inner join 规则_诊疗限制 t on upper(g.代码) like t.诊疗编码 || '%' and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0 
...
--脚本：跑测重复计费的透析项目
delete from 应用_问题线索 where 机构编码 = 'H00000000000' and 就医来源 = '门诊' and 项目来源 = '诊疗' 
    and 线索来源 = '透析专项' and 问题类型 = '重复计费诊疗';
commit;

--脚本：创建操作项目临时表
create global temporary table 临时_透析项目 as 
select a.机构编码,a.名称,a.身份证号,a.日期,sum(a.人次) 人次,b.子项编码,b.子项名称
from 统计_门诊频度 a inner join 规则_诊疗重复 b on regexp_instr(b.领域标识,'(透析|血透|腹透)') > 0 
...
inner join 规则_诊疗重复 t on (regexp_instr(upper(g.代码), t.子项编码) > 0 or regexp_instr(g.名称, t.子项名称) > 0) 
    and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0 
where exists (select 1 from 临时_透析项目 s where g.机构编码 = s.机构编码 and g.身份证号 = s.身份证号 and g.日期 = s.日期 and g.名称 <> s.名称
    and (regexp_instr(upper(g.代码), s.子项编码) > 0 or regexp_instr(g.名称, s.子项名称) > 0))
order by 门诊科室,身份证号,门诊日期,名称;
commit;

--删除临时表
drop table 临时_透析项目 purge;
-----------------------------------------------------------------------------------------------

--脚本：跑测关联缺失的透析项目
delete from 应用_问题线索 where 机构编码 = 'H00000000000' and 就医来源 = '门诊' and 项目来源 = '诊疗' 
    and 线索来源 = '透析专项' and 问题类型 = '诊疗关联缺失';
commit;
...
where a.类别 not in ('西药费','成药费','草药费','材料费') and regexp_instr(b.领域标识,'(透析|血透|腹透)') > 0
...
inner join 规则_诊疗关联 t on upper(g.代码) like t.主项编码 || '%' and g.名称 like '%' || t.主项名称 || '%' 
    and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0 
where not exists (select 1 from 临时_透析项目 s where g.机构编码 = s.机构编码 and g.身份证号 = s.身份证号 and g.日期 = s.日期 
...
from 统计_门诊频度 a inner join 规则_诊疗分解 b on regexp_instr(b.领域标识,'(透析|血透|腹透)') > 0
...
select '门诊' 就医来源,
    ...
    类别, 代码, 名称, 规格, 单位, 单价, 人次, 天数, 频次, 剂量, 数量, 金额, 列支, 日期
from 临时_门诊诊疗 g
inner join 规则_诊疗分解 t on regexp_instr(g.名称, t.分项名称) > 0 and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0 
where exists (select 1 from 临时_透析项目 s where g.机构编码 = s.机构编码 and g.身份证号 = s.身份证号 and g.日期 = s.日期 and g.名称 = s.名称)
order by 门诊科室,身份证号,门诊日期,名称;
commit;
...
--脚本：跑测分解项目的透析项目
delete from 应用_问题线索 where 机构编码 = 'H00000000000' and 就医来源 = '门诊' and 项目来源 = '材料' 
    and 线索来源 = '透析专项' and 问题类型 = '分解计费材料';
commit;

--脚本：创建临时表
create global temporary table 临时_透析项目 as 
select a.机构编码,a.名称,a.身份证号,a.日期,sum(a.人次) 人次,b.分项编码,b.分项名称
from 统计_门诊频度 a inner join 规则_材料分解 b on regexp_instr(b.领域标识,'(透析|血透|腹透)') > 0
and regexp_instr(upper(a.代码),b.分项编码) > 0 and regexp_instr(a.名称,b.分项名称) > 0
...
select '门诊' 就医来源,
    ...
    类别, 代码, 名称, 规格, 单位, 单价, 人次, 天数, 频次, 剂量, 数量, 金额, 列支, 日期
from 临时_门诊材料 g
inner join 规则_材料分解 t on regexp_instr(g.名称, t.分项名称) > 0 and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0
where exists (select 1 from 临时_透析项目 s where g.机构编码 = s.机构编码 and g.身份证号 = s.身份证号 and g.日期 = s.日期 and g.名称 = s.名称)
order by 门诊科室,身份证号,门诊日期,名称;
commit;
...
inner join 规则_药品限制 t on regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0
...
inner join 规则_药品限制 t on upper(g.代码) like t.药品编码 || '%' and t.操作限制 is not null 
    and regexp_instr(t.领域标识,'(透析|血透|腹透)') > 0 
where not exists (select 1 from 临时_透析项目 s where g.机构编码 = s.机构编码 and g.身份证号 = s.身份证号
