...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '就医耗材率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '性别耗材率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '年龄耗材率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '持证耗材率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '地域耗材率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '病种耗材率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '专业耗材率';
commit;
...
select a.机构编码
    ...
    ,a.住院医生
    ,b.项目名称
    ,round(sum(b.项目数量) / count(distinct a.住院医生),2) 耗材率
from 模型_住院人群 a inner join 模型_住院项目 b on a.机构编码 = b.机构编码 and a.就医方式 = b.就医方式  and a.身份证号 = b.身份证号 and a.住院日期 = b.住院日期
where a.机构编码 = 'H00000000000' and b.项目类别 in ('材料费') 
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 耗材率) 
    ...
    ,项目名称
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.住院医生 = b.住院医生 and a.项目名称 = b.项目名称 and a.耗材率 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.住院医生 = t.住院医生 and s.项目名称 = t.项目名称);
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '医生耗材率';
commit;
...
select a.机构编码
    ...
    ,a.耗材率 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.住院医生 = b.住院医生 and a.项目名称 = b.项目名称 and a.耗材率 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
...
select a.机构编码
    ...
    ,a.住院科室
    ,b.项目名称
    ,round(sum(b.项目数量) / count(distinct a.住院科室),2) 耗材率
from 模型_住院人群 a inner join 模型_住院项目 b on a.机构编码 = b.机构编码 and a.就医方式 = b.就医方式  and a.身份证号 = b.身份证号 and a.住院日期 = b.住院日期
where a.机构编码 = 'H00000000000' and b.项目类别 in ('材料费') 
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 耗材率) 
    ...
    ,项目名称
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.住院科室 = b.住院科室 and a.项目名称 = b.项目名称 and a.耗材率 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.住院科室 = t.住院科室 and s.项目名称 = t.项目名称);
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '科室耗材率';
commit;
...
select a.机构编码
    ...
    ,a.耗材率 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.住院科室 = b.住院科室 and a.项目名称 = b.项目名称 and a.耗材率 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
