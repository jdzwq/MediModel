...
--注意：
delete from 负荷_机构摘要 where 机构编码 = 'H33018205257';
commit;
...
SELECT trim(结算流水号) 流水号,sum(医疗金额) 医疗金额 from 洋溪卫生服务中心_门诊就医@USR_LINK
...
--注意：无效的结算记录不会被载入
delete from 负荷_门诊就医 where 机构编码 = 'H33018205257';
commit;

insert into 负荷_门诊就医 (机构等级, 机构编码, 机构名称, 险种类别, 持证类别, 参保区划, 姓名, 性别, 年龄, 身份证号, 
...
select distinct a.机构等级, trim(a.机构编码), a.机构名称, a.险种类别, a.持证类别, a.参保地区划, a.人员姓名, a.人员性别, a.人员年龄, trim(a.身份证号), 
       trim(a.门诊号), a.门诊日期, a.门诊科室名称, a.首诊医生姓名, a.疾病编码icd, a.疾病诊断名称, a.次要诊断组合, trim(a.结算流水号), 
       a.结算类别, a.结算日期, a.医疗金额, a.医保范围金额, a.基金支付, a.个账支付, a.现金支付
from 洋溪卫生服务中心_门诊就医@USR_LINK a inner join 洋溪卫生服务中心_机构摘要@USR_LINK b on a.机构编码 = b.机构编码
...
--注意：无效的结算记录不会被载入
delete from 负荷_门诊结算 where 机构编码 = 'H33018205257';
commit;
...
from 洋溪卫生服务中心_门诊结算@USR_LINK a
where exists (select 1 from 洋溪卫生服务中心_机构摘要@USR_LINK b where a.机构编码 = b.机构编码)
and not exists (select 1 from 临时_无效结算 c where trim(a.结算流水号) = c.流水号);
commit;

alter index 索引_负荷_门诊结算 rebuild;
alter index 索引_负荷_门诊结算_国标代码 rebuild;
--*******************************************************************************************--
...
delete from 负荷_门诊处方 where 机构编码 = 'H33018205257';
commit;
...
delete from 负荷_门诊手术 where 机构编码 = 'H33018205257';
commit;
...
delete from 负荷_门诊麻醉 where 机构编码 = 'H33018205257';
commit;
...
delete from 负荷_检查登记 where 机构编码 = 'H33018205257' and 就医方式 = '门诊';
commit;
...
delete from 负荷_检查报告 where 机构编码 = 'H33018205257' and 就医方式 = '门诊';
commit;
...
delete from 负荷_化验登记 where 机构编码 = 'H33018205257' and 就医方式 = '门诊';
commit;
...
delete from 负荷_化验报告 where 机构编码 = 'H33018205257' and 就医方式 = '门诊';
commit;
