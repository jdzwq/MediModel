...
from 统计_门诊频度 a inner join 规则_诊疗重复 b on regexp_instr(b.领域标识,'(介入治疗)') > 0 
...
from 统计_门诊频度 a inner join 规则_诊疗分解 b on regexp_instr(b.领域标识,'(介入治疗)') > 0
...
create global temporary table 临时_介入项目 as 
select a.机构编码,a.名称,a.身份证号,a.日期,sum(a.人次) 人次,b.分项编码,b.分项名称
from 统计_门诊频度 a inner join 规则_材料分解 b on regexp_instr(b.领域标识,'(介入治疗)') > 0
and regexp_instr(upper(a.代码),b.分项编码) > 0 and regexp_instr(a.名称,b.分项名称) > 0
