/*********************************************************************************************
脚本说明：以下脚本用于分析门诊人群流动水位指标，运行时应替换实际机构代码和名称
机构编码：H00000000000
机构名称：测试医院
**********************************************************************************************/
...
(
select distinct 就医方式, to_char(门诊日期,'YYYYMM') 年月, count(distinct 身份证号) 人数, 0 增量, 0 减量, 0.0 置换率
from 模型_门诊人群
where 机构编码 = 'H00000000000' 
...
BEGIN
  ...
  LOOP
    ...
    update TMP_水位 set 增量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        where 就医方式 = rec.就医方式 and to_char(a.门诊日期,'YYYYMM') = rec.年月
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and to_char(b.门诊日期,'YYYYMM') < rec.年月 and a.身份证号 = b.身份证号))
    where 就医方式 = rec.就医方式 and 年月 = rec.年月;
    commit;
   
    update TMP_水位 set 减量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        where 就医方式 = rec.就医方式 and to_char(a.门诊日期,'YYYYMM') < rec.年月
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and to_char(b.门诊日期,'YYYYMM') = rec.年月 and a.身份证号 = b.身份证号))
    where 就医方式 = rec.就医方式 and 年月 = rec.年月;
    commit;
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '就医流动率';
commit;
...
select t.*, rownum 月数 from
(
select distinct 就医方式, 性别对象, to_char(门诊日期,'YYYYMM') 年月, count(distinct 身份证号) 人数, 0 增量, 0 减量, 0.0 置换率
from 模型_门诊人群
where 机构编码 = 'H00000000000' 
...
BEGIN
  ...
  LOOP
    ...
    update TMP_水位 set 增量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        ...
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and a.性别对象 = b.性别对象 and to_char(b.门诊日期,'YYYYMM') < rec.年月 and a.身份证号 = b.身份证号))
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '性别流动率';
commit;
...
select t.*, rownum 月数 from
(
select distinct 就医方式, 年龄对象, to_char(门诊日期,'YYYYMM') 年月, count(distinct 身份证号) 人数, 0 增量, 0 减量, 0.0 置换率
from 模型_门诊人群
where 机构编码 = 'H00000000000' 
...
BEGIN
  ...
  LOOP
    ...
    update TMP_水位 set 增量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        ...
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and a.年龄对象 = b.年龄对象 and to_char(b.门诊日期,'YYYYMM') < rec.年月 and a.身份证号 = b.身份证号))
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '年龄流动率';
commit;
...
select t.*, rownum 月数 from
(
select distinct 就医方式, 持证类别, to_char(门诊日期,'YYYYMM') 年月, count(distinct 身份证号) 人数, 0 增量, 0 减量, 0.0 置换率
from 模型_门诊人群
where 机构编码 = 'H00000000000' 
...
BEGIN
  ...
  LOOP
    ...
    update TMP_水位 set 增量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        ...
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and a.持证类别 = b.持证类别 and to_char(b.门诊日期,'YYYYMM') < rec.年月 and a.身份证号 = b.身份证号))
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '持证流动率';
commit;
...
select t.*, rownum 月数 from
(
select distinct 就医方式, 参保地域, to_char(门诊日期,'YYYYMM') 年月, count(distinct 身份证号) 人数, 0 增量, 0 减量, 0.0 置换率
from 模型_门诊人群
where 机构编码 = 'H00000000000' 
...
BEGIN
  ...
  LOOP
    ...
    update TMP_水位 set 增量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        ...
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and a.参保地域 = b.参保地域 and to_char(b.门诊日期,'YYYYMM') < rec.年月 and a.身份证号 = b.身份证号))
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '地域流动率';
commit;
...
select t.*, rownum 月数 from
(
select distinct 就医方式, 病种名称, to_char(门诊日期,'YYYYMM') 年月, count(distinct 身份证号) 人数, 0 增量, 0 减量, 0.0 置换率
from 模型_门诊人群
where 机构编码 = 'H00000000000' 
...
BEGIN
  ...
  LOOP
    ...
    update TMP_水位 set 增量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        ...
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and a.病种名称 = b.病种名称 and to_char(b.门诊日期,'YYYYMM') < rec.年月 and a.身份证号 = b.身份证号))
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '病种流动率';
commit;
...
select t.*, rownum 月数 from
(
select distinct 就医方式, 专业名称, to_char(门诊日期,'YYYYMM') 年月, count(distinct 身份证号) 人数, 0 增量, 0 减量, 0.0 置换率
from 模型_门诊人群
where 机构编码 = 'H00000000000' 
...
BEGIN
  ...
  LOOP
    ...
    update TMP_水位 set 增量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        ...
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and a.专业名称 = b.专业名称 and to_char(b.门诊日期,'YYYYMM') < rec.年月 and a.身份证号 = b.身份证号))
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '专业流动率';
commit;
...
select t.*, rownum 月数 from
(
select distinct 就医方式, 门诊医生, to_char(门诊日期,'YYYYMM') 年月, count(distinct 身份证号) 人数, 0 增量, 0 减量, 0.0 置换率
from 模型_门诊人群
where 机构编码 = 'H00000000000' 
...
BEGIN
  ...
  LOOP
    ...
    update TMP_水位 set 增量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        ...
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and a.门诊医生 = b.门诊医生 and to_char(b.门诊日期,'YYYYMM') < rec.年月 and a.身份证号 = b.身份证号))
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 置换率) 
    ...
    ,门诊医生
FROM TMP_水位 a where 1=1
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.门诊医生 = b.门诊医生 and a.置换率 <= b.中分位)
)t
where s.就医方式 = t.就医方式 and s.门诊医生 = t.门诊医生);
commit;
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '医生流动率';
commit;
...
select 'H00000000000' 机构编码
    ...
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where 1=1
and a.就医方式 = b.就医方式 and a.门诊医生 = b.门诊医生 and a.置换率 > b.低分位 + b.分位矩 * 1.5
order by 就医来源,特征位;
commit;
...
select t.*, rownum 月数 from
(
select distinct 就医方式, 门诊科室, to_char(门诊日期,'YYYYMM') 年月, count(distinct 身份证号) 人数, 0 增量, 0 减量, 0.0 置换率
from 模型_门诊人群
where 机构编码 = 'H00000000000' 
...
BEGIN
  ...
  LOOP
    ...
    update TMP_水位 set 增量 = (select count(distinct 身份证号) from 模型_门诊人群 a 
        ...
        and not exists (select 1 from 模型_门诊人群 b where a.就医方式 = b.就医方式 and a.门诊科室 = b.门诊科室 and to_char(b.门诊日期,'YYYYMM') < rec.年月 and a.身份证号 = b.身份证号))
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 置换率) 
    ...
    ,门诊科室
FROM TMP_水位 a where 1=1
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.门诊科室 = b.门诊科室 and a.置换率 <= b.中分位)
)t
where s.就医方式 = t.就医方式 and s.门诊科室 = t.门诊科室);
commit;
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '科室流动率';
commit;
...
select 'H00000000000' 机构编码
    ...
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where 1=1
and a.就医方式 = b.就医方式 and a.门诊科室 = b.门诊科室 and a.置换率 > b.低分位 + b.分位矩 * 1.5
order by 就医来源,特征位;
commit;
