...
SELECT 
	...
	case when substr(a.CERTNO,7,1) = '*' then a.PSN_NO else trim(a.CERTNO) end 身份证号,
	substr(a.mdtrt_id,1,50) as 门诊号,
	trunc(to_timestamp(a.begntime,'YYYY-MM-DD HH24:MI:SS.FF3')) as 门诊日期,
	substr(a.DSCG_DEPT_NAME,1,100) as 门诊科室名称,
	substr(a.CHFPDR_NAME,1,100) as 首诊医生姓名,
	substr(a.DISE_NO,1,100) as 疾病编码ICD,
	substr(a.DISE_NAME,1,200) as 疾病诊断名称,
	'' as 次要诊断组合,
	substr(b.SETL_ID,1,50) as 结算流水号,
	CASE WHEN b.med_type in ('11','12','120','13','110101','1102','9211') THEN '门诊' 
		WHEN b.med_type in ('140104','140201','140401','991702','992102') THEN '慢特' 
		WHEN b.med_type in ('28') THEN '日间' 
		WHEN b.med_type in ('21','210104','52') THEN '住院' 
        WHEN b.med_type in ('52') THEN '生育' 
		WHEN b.med_type in ('41') THEN '购药' 
		WHEN b.med_type in ('71') THEN '家床' 
		ELSE b.med_type END as 结算类别,
	to_timestamp(b.setl_time,'YYYY-MM-DD HH24:MI:SS.FF3') as 结算日期,
...
WHERE b.med_type NOT IN ('21','210104','52');
commit;
...
and a.med_type NOT IN ('21','210104','52')
group by b.MDTRT_ID;
...
select 
	...
	to_timestamp(b.fee_ocur_time,'YYYY-MM-DD HH24:MI:SS.FF3') as 费用发生时间,
...
WHERE a.med_type NOT IN ('21','210104''52');
commit;
...
SELECT 
	...
	trunc(to_timestamp(a.begntime,'YYYY-MM-DD HH24:MI:SS.FF3')) as 入院日期,
	trunc(to_timestamp(a.endtime,'YYYY-MM-DD HH24:MI:SS.FF3')) as 出院日期,
	...
	to_number(b.cash_payamt) as 现金支付
FROM MDTRT_D a INNER JOIN SETL_D b ON a.fixmedins_code = b.fixmedins_code and a.mdtrt_id = b.mdtrt_id
WHERE b.med_type IN ('21','210104','52');
commit;
...
from MDTRT_D a,SETL_LIST_DIAG_D b 
where a.psn_no = b.psn_no and a.MDTRT_ID = b.MDTRT_ID
and a.med_type IN ('21','210104','52')
group by b.MDTRT_ID;
...
SELECT 
	...
	'' as 愈合类型
FROM MDTRT_D a INNER JOIN SETL_LIST_OPRN_D b ON a.psn_no = b.psn_no and a.mdtrt_id = b.mdtrt_id
WHERE a.med_type IN ('21','210104','52');
commit;
