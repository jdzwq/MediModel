...
--脚本：创建操作项目临时表
create global temporary table 临时_康复理疗项目 as 
select a.机构编码,a.名称,a.身份证号,a.日期,sum(a.人次) 人次,b.子项编码,b.子项名称
from 统计_住院频度 a inner join 规则_诊疗重复 b on regexp_instr(b.领域标识,'(康复治疗|物理治疗)') > 0 
...
from 统计_住院频度 a inner join 规则_诊疗分解 b on regexp_instr(b.领域标识,'(康复治疗|物理治疗)') > 0
...
create global temporary table 临时_康复理疗项目 as 
select a.机构编码,a.名称,a.身份证号,a.日期,sum(a.人次) 人次,b.分项编码,b.分项名称
from 统计_住院频度 a inner join 规则_材料分解 b on regexp_instr(b.领域标识,'(康复治疗|物理治疗)') > 0
and regexp_instr(upper(a.代码),b.分项编码) > 0 and regexp_instr(a.名称,b.分项名称) > 0
