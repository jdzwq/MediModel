...
GROUP BY 机构编码,机构名称,业务周期,代码,类别,名称,规格,单价,数量单位
having sum(数量) > 0;
...
from 负荷_药库消耗 where 机构编码 = 'H33018205257' and 业务类型 in ('物资采购') and 数量 > 0
/*union all
select 机构编码,机构名称,国标代码 as 代码,拆零比,
       数量,数量*购入价 as 购入金额,购入价, 包装单位 as 购入单位, to_char(业务日期,'YYYY') as 业务期间
from 负荷_药房消耗 where 机构编码 = 'H33018205257' and 业务类型 in ('物资采购') and 数量 > 0*/
) a
...
from(
select 机构编码,机构名称,国标代码 as 代码,单位,数量,零售价,to_char(业务日期,'YYYY') as 业务期间
from 负荷_药房消耗 where 机构编码 = 'H33018205257' and 业务类型 in ('门诊销售','住院销售')
) a
...
delete from 统计_库房进销 where 机构编码 = 'H33018205257';
commit;

INSERT INTO 统计_库房进销 (机构编码, 机构名称, 业务期间, 代码, 类别, 名称, 规格, 计费人次, 计费数量, 计费金额, 计费均价, 计费单位)
select 机构编码, 机构名称, 业务周期, 代码, 类别, 名称, 规格, 计费人次, 计费数量, 计费金额, 计费均价, 计费单位
from 临时_物资计费
ORDER BY 业务周期, 代码;
commit;
...
begin
  for cur in (select a.rowid, b.购入数量,b.购入金额,b.购入均价,b.购入单位,b.拆零比
    ...
    where a.机构编码 = 'H33018205257' and a.机构编码 = b.机构编码 and a.业务期间 = b.业务期间 and a.代码 = b.代码) loop
    update 统计_库房进销 
    set 购入数量 = cur.购入数量,
    购入金额 = cur.购入金额,
    购入均价 = cur.购入均价,
