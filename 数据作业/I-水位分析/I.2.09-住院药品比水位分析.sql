...
select 机构编码, 就医方式, to_char(住院日期,'YYYYMM') 年月, round(sum(项目金额) / sum(医疗金额),2) 药品比
from(
select a.机构编码
    ,a.就医方式
    ,a.住院日期
    ,a.医疗金额
    ,(select sum(b.项目金额) from 模型_住院项目 b where a.机构编码 = b.机构编码 and a.就医方式 = b.就医方式  and a.身份证号 = b.身份证号 and a.住院日期 = b.住院日期 and b.项目类别 in ('西药费','成药费','草药费')) 项目金额
from 模型_住院人群 a 
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '就医药品比';
commit;
...
drop table TMP_水位 purge;
create table TMP_水位 nologging as
select 机构编码, 就医方式, 性别对象, to_char(住院日期,'YYYYMM') 年月, round(sum(项目金额) / sum(医疗金额),2) 药品比
from(
select a.机构编码
    ,a.就医方式
    ,a.性别对象
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '性别药品比';
commit;
...
drop table TMP_水位 purge;
create table TMP_水位 nologging as
select 机构编码, 就医方式, 年龄对象, to_char(住院日期,'YYYYMM') 年月, round(sum(项目金额) / sum(医疗金额),2) 药品比
from(
select a.机构编码
    ,a.就医方式
    ,a.年龄对象
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '年龄药品比';
commit;
...
drop table TMP_水位 purge;
create table TMP_水位 nologging as
select 机构编码, 就医方式, 持证类别, to_char(住院日期,'YYYYMM') 年月, round(sum(项目金额) / sum(医疗金额),2) 药品比
from(
select a.机构编码
    ,a.就医方式
    ,a.持证类别
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '持证药品比';
commit;
...
drop table TMP_水位 purge;
create table TMP_水位 nologging as
select 机构编码, 就医方式, 参保地域, to_char(住院日期,'YYYYMM') 年月, round(sum(项目金额) / sum(医疗金额),2) 药品比
from(
select a.机构编码
    ,a.就医方式
    ,a.参保地域
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '地域药品比';
commit;
...
drop table TMP_水位 purge;
create table TMP_水位 nologging as
select 机构编码, 就医方式, 病种名称, to_char(住院日期,'YYYYMM') 年月, round(sum(项目金额) / sum(医疗金额),2) 药品比
from(
select a.机构编码
    ,a.就医方式
    ,a.病种名称
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '病种药品比';
commit;
...
drop table TMP_水位 purge;
create table TMP_水位 nologging as
select 机构编码, 就医方式, 专业名称, to_char(住院日期,'YYYYMM') 年月, round(sum(项目金额) / sum(医疗金额),2) 药品比
from(
select a.机构编码
    ,a.就医方式
    ,a.专业名称
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '专业药品比';
commit;
...
drop table TMP_水位 purge;
create table TMP_水位 nologging as
select 机构编码, 就医方式, 住院医生, to_char(住院日期,'YYYYMM') 年月, round(sum(项目金额) / sum(医疗金额),2) 药品比
from(
select a.机构编码
    ,a.就医方式
    ,a.住院医生
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 药品比) 
    ...
    ,住院医生
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.住院医生 = b.住院医生 and a.药品比 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.住院医生 = t.住院医生);
commit;
...
select a.机构编码
    ...
    ,a.药品比 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.住院医生 = b.住院医生 and a.药品比 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
...
drop table TMP_水位 purge;
create table TMP_水位 nologging as
select 机构编码, 就医方式, 住院科室, to_char(住院日期,'YYYYMM') 年月, round(sum(项目金额) / sum(医疗金额),2) 药品比
from(
select a.机构编码
    ,a.就医方式
    ,a.住院科室
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 药品比) 
    ...
    ,住院科室
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.住院科室 = b.住院科室 and a.药品比 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.住院科室 = t.住院科室);
commit;
...
select a.机构编码
    ...
    ,a.药品比 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.住院科室 = b.住院科室 and a.药品比 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
