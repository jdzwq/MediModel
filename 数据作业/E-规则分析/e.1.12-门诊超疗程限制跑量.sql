...
select 机构编码, 身份证号, 门诊日期, 类别, 代码, 名称, 规格, 单位, 单价, to_char(日期,'YYYYMM') 月次, sum(天数) 天数, to_number(regexp_substr(疗程限制, '[0-9]+(\.[0-9]+)?', 1)) 限制
from 模型_门诊频度 a inner join 规则_药品限制 t on a.代码 like t.药品编码 || '%' and t.疗程限制 is not null
--and regexp_instr(t.领域标识,'(康复|物理|中医)') > 0
where a.机构编码 = 'H00000000000' and a.类别 in ('西药费','成药费') 
group by 机构编码, 身份证号, 门诊日期, 类别, 代码, 名称, 规格, 单位, 单价, to_char(日期,'YYYYMM'), to_number(regexp_substr(疗程限制, '[0-9]+(\.[0-9]+)?', 1))
...
create table 临时_计费项目 NOLOGGING as 
select 机构编码, 身份证号, 门诊日期, 类别, 代码, 名称, 规格, 单位, 单价, to_char(日期,'YYYYMM') 月次, sum(天数) 天数, to_number(regexp_substr(疗程限制, '[0-9]+(\.[0-9]+)?', 1)) 限制
from 模型_门诊频度 a inner join 规则_诊疗限制 t on a.代码 like t.诊疗编码 || '%' and t.疗程限制 is not null
--and regexp_instr(t.领域标识,'(康复|物理|中医)') > 0
