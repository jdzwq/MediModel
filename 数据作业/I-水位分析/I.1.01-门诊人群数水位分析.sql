/*********************************************************************************************
脚本说明：以下脚本用于分析门诊人群规模水位指标，运行时应替换实际机构代码和名称
机构编码：H00000000000
机构名称：测试医院
**********************************************************************************************/
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '就医人群数';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '性别人群数';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '年龄人群数';
commit;
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '持证人群数';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '地域人群数';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '专业人群数';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '病种人群数';
commit;
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 人数) 
    ...
    ,门诊医生
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.门诊医生 = b.门诊医生 and a.人数 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.门诊医生 = t.门诊医生);
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '科室人群数';
commit;
...
select a.机构编码
    ...
    ,a.人数 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.门诊医生 = b.门诊医生 and a.人数 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 人数) 
    ...
    ,门诊科室
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.门诊科室 = b.门诊科室 and a.人数 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.门诊科室 = t.门诊科室);
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '医生人群数';
commit;
...
select a.机构编码
    ...
    ,a.人数 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.门诊科室 = b.门诊科室 and a.人数 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
