...
--*******************************************************************************************--
--脚本：从门诊负荷表中归集门诊诊次
delete from 统计_门诊诊次 where 机构编码 = 'H33018205257';
commit;
...
SELECT 机构等级，机构编码, 机构名称, 姓名, 性别, 
    ...
    个账支付,
    现金支付
FROM 负荷_门诊就医 where 机构编码 = 'H33018205257'
) a
group by 机构等级, 机构编码, 机构名称, 姓名, 性别, 年龄, 身份证号, 险种类别, 持证类别, 门诊日期, 门诊天数
having sum(医疗金额) <> 0;
commit;

alter index 索引_统计_门诊诊次 rebuild;
--*******************************************************************************************--
--脚本：从门诊负荷表中归集门诊项目频度
delete from 统计_门诊频度 where 机构编码 = 'H33018205257';
commit;
...
where b.机构编码 = 'H33018205257' and nvl(a.数量,0) <> 0
group by b.机构编码, b.机构名称, b.姓名, b.身份证号, trunc(b.门诊日期),a.国标代码, a.收费项目类别, a.医疗属性类别,
    a.国标名称,a.规格,a.数量单位,a.单价,trunc(a.费用发生日期)
having sum(a.数量) <> 0;
commit;

alter index 索引_统计_门诊频度 rebuild;
--*******************************************************************************************--
--脚本：从门诊负荷表中归集门诊结构
delete from 统计_门诊结构 where 机构编码 = 'H33018205257';
commit;
...
alter index 索引_统计_门诊结构 rebuild;
--*******************************************************************************************--
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     ...
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('床位费') group by a.rowid)loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('诊查费','挂号费','会诊费','巡诊费') group by a.rowid)loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('护理费','注射费') group by a.rowid)loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('化验费','病理费') group by a.rowid)loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('检查费') group by a.rowid)loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('吸氧费','抢救费','急救费','一般诊疗费') group by a.rowid)loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期   
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('手术费','麻醉费')group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('介入治疗费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('放射治疗费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('物理治疗费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('中医治疗费','中医论治费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('康复治疗费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('精神治疗费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('西药费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('成药费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('草药费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('材料费') group by a.rowid )loop
...
begin
  for cur in (select a.rowid, sum(b.金额) 金额,sum(b.列支) 列支,SUM(b.数量) 数量,count(distinct 名称) 项数,sum(频次) 次数
     from 统计_门诊结构 A inner join 统计_门诊频度 B on A.机构编码 = B.机构编码 AND A.身份证号 = B.身份证号 and A.门诊日期 = B.门诊日期
     WHERE a.机构编码 = 'H33018205257' AND b.类别 IN ('其他服务费','体检费','疫苗费') group by a.rowid )loop
