...
select 机构编码
    ...
    ,round(count(distinct 身份证号 || to_char(住院日期,'YYYYMMDD')) / count(distinct 身份证号),2) 人次比
from 模型_住院人群
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '就医人次比';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '性别人次比';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '年龄人次比';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '持证人次比';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '地域人次比';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '专业人次比';
commit;
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '病种人次比';
commit;
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 人次比) 
    ...
    ,住院医生
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.住院医生 = b.住院医生 and a.人次比 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.住院医生 = t.住院医生);
commit;
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '医生人次比';
commit;
...
select a.机构编码
    ...
    ,a.人次比 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.住院医生 = b.住院医生 and a.人次比 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 人次比) 
    ...
    ,住院科室
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.住院科室 = b.住院科室 and a.人次比 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.住院科室 = t.住院科室);
commit;
...
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('住院') and 特征类 = '科室人次比';
commit;
...
select a.机构编码
    ...
    ,a.人次比 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.住院科室 = b.住院科室 and a.人次比 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
