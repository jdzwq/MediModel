/*********************************************************************************************
脚本说明：以下脚本用于分析门诊药品使用水位指标，运行时应替换实际机构代码和名称
机构编码：H00000000000
机构名称：测试医院
**********************************************************************************************/
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '就医用药率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '性别用药率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '年龄用药率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '持证用药率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '地域用药率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '病种用药率';
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '专业用药率';
commit;
...
select a.机构编码
    ...
    ,a.门诊医生
    ,b.项目名称
    ,round(sum(b.项目次数) / count(distinct a.门诊医生),2) 用药率
from 模型_门诊人群 a inner join 模型_门诊项目 b on a.机构编码 = b.机构编码 and a.就医方式 = b.就医方式  and a.身份证号 = b.身份证号 and a.门诊日期 = b.门诊日期
where a.机构编码 = 'H00000000000' and b.项目类别 in ('西药费','成药费') 
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 用药率) 
    ...
    ,项目名称
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.门诊医生 = b.门诊医生 and a.项目名称 = b.项目名称 and a.用药率 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.门诊医生 = t.门诊医生 and s.项目名称 = t.项目名称);
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '医生用药率';
commit;
...
select a.机构编码
    ...
    ,a.用药率 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.门诊医生 = b.门诊医生 and a.项目名称 = b.项目名称 and a.用药率 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
...
select a.机构编码
    ...
    ,a.门诊科室
    ,b.项目名称
    ,round(sum(b.项目次数) / count(distinct a.门诊科室),2) 用药率
from 模型_门诊人群 a inner join 模型_门诊项目 b on a.机构编码 = b.机构编码 and a.就医方式 = b.就医方式  and a.身份证号 = b.身份证号 and a.门诊日期 = b.门诊日期
where a.机构编码 = 'H00000000000' and b.项目类别 in ('西药费','成药费') 
...
SELECT distinct PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY 用药率) 
    ...
    ,项目名称
FROM TMP_水位 a where 机构编码 = 'H00000000000'
and exists (select 1 from TMP_分位 b where a.就医方式 = b.就医方式 and a.门诊科室 = b.门诊科室 and a.项目名称 = b.项目名称 and a.用药率 <= b.中分位)
)t where s.就医方式 = t.就医方式 and s.门诊科室 = t.门诊科室 and s.项目名称 = t.项目名称);
commit;
...
--筛选高水位
delete from 线索_问题特征 where 机构编码 = 'H00000000000' and 就医来源 in ('门诊','购药') and 特征类 = '科室用药率';
commit;
...
select a.机构编码
    ...
    ,a.用药率 特征值
    ,年月 特征位 ,'高水位' 特征区 from TMP_水位 a, TMP_分位 b
where a.就医方式 = b.就医方式 and a.门诊科室 = b.门诊科室 and a.项目名称 = b.项目名称 and a.用药率 > b.低分位 + b.分位矩 * 1.5
order by 特征名,特征位;
commit;
