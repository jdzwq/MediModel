...
where a.机构编码 = 'P33018203895' and a.类别 in ('西药费','成药费')
...
select a.机构编码, a.机构名称, b.门诊科室, b.门诊医生, b.身份证号, b.姓名, b.性别, b.年龄, b.门诊日期, b.门诊天数, b.疾病诊断,
    ...
    a.日期, b.机构等级
from 统计_门诊频度 a inner join 统计_门诊诊次 b on a.机构编码 = b.机构编码 and a.身份证号 = b.身份证号 and a.门诊日期 = b.门诊日期
where a.机构编码 = 'P33018203895' and a.类别 in ('材料费')
group by a.机构编码, a.机构名称, b.门诊科室, b.门诊医生, b.身份证号, b.姓名, b.性别, b.年龄, b.门诊日期, b.门诊天数, b.疾病诊断,
    a.类别, a.代码,a.名称, a.规格, a.单位, a.单价, a.日期, b.机构等级
order by a.代码;
...
delete from 应用_问题线索 where 机构编码 = 'P33018203895' and 就医来源 = '门诊' and 项目来源 = '药品' 
...
from 临时_门诊药品 g inner join 规则_药品限制 t on upper(g.代码) like t.药品编码 || '%' and t.机构限制 is not null 
    and decode(g.机构等级,'三级',5,'二级',4,'一级',3,'社区',3,'诊所',2,'药店',1,0) < decode(t.机构限制,'三级',5,'二级',4,0)
...
select '门诊' 就医来源,
    ...
    机构编码, 机构名称,门诊科室, 门诊医生, 身份证号, 姓名, 性别, 年龄, 门诊日期, 门诊天数, 疾病诊断,
    类别, 代码, 名称, 规格, 单位, 单价, 人次, 天数, 频次, 剂量, 数量, 金额, 列支, 日期
from 临时_门诊药品 g where g.年龄 not in ('婴幼儿')
...
select '门诊' 就医来源,
    ...
    机构编码, 机构名称,门诊科室, 门诊医生, 身份证号, 姓名, 性别, 年龄, 门诊日期, 门诊天数, 疾病诊断,
    类别, 代码, 名称, 规格, 单位, 单价, 人次, 天数, 频次, 剂量, 数量, 金额, 列支, 日期
from 临时_门诊药品 g  where g.年龄 not in ('婴幼儿','少年儿童')
...
select '门诊' 就医来源,
    ...
    '药店专项' 线索来源,
    '超加价标准药品' 问题类型,
    '该药品购入价' || t.购入均价 || '，销售价' || g.单价 || '加成比超过1.725倍' 问题情形,
    '超标准支付' 问题性质,
    t.购入均价 问题数量, 
    round((g.单价 - t.购入均价 * 1.5 * 1.15) * g.数量,2) 问题金额,
...
from 临时_门诊药品 g  inner join 统计_库房进销 t on g.机构编码 = t.机构编码 and to_char(g.门诊日期,'YYYY') = t.业务期间
    and g.代码 = t.代码 and g.单价 > t.购入均价 * 1.5 * 1.15
and exists (select 1 from 字典_常用药品 c where g.代码 = c.药品编码)
order by 门诊科室,身份证号,门诊日期,名称;
commit;
--*******************************************************************************************--
